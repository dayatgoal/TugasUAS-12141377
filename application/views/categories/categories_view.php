...
<div class="col-md-2">
		  ...
		  <?php 
			$jumlahProduk = $listCategories->num_rows(); //$listProduct berasal dari data yang dilempar dari controller, yaitu $data['listProducts']. num_rows() digunakan untuk menghitung jumlah baris yang dimiliki ketika kita melakukan select dari database

			if($jumlahProduk == 0){
		?>
			<!-- Kalau datanya masih kosong, kita harus melakukan add product -->
			<a href="<?= base_url() ?>index.php/categories/addCategory">Tambah Produk</a>
		<?php
			}
			else {
		?>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<a href="<?= base_url() ?>categories/updateCategory/<?= $row->CategoryID ?>">Update</a> 
							 
							<a href="<?= base_url() ?>categories/deleteCategoryDb/<?= $row->CategoryID ?>">Delete</a>
						</td>
					</tr>
					<?php
						}
